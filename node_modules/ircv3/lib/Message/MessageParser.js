...
function parsePrefix(raw) {
    const [nick, hostName] = shared_utils_1.splitWithLimit(raw, '!', 2);
    if (hostName) {
        const [user, host] = shared_utils_1.splitWithLimit(hostName, '@', 2);
        if (host) {
...
function parseTags(raw) {
    ...
    for (const tagString of tagStrings) {
        const [tagName, tagValue] = shared_utils_1.splitWithLimit(tagString, '=', 2);
        if (tagName === '') {
