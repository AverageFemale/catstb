...
class IrcClient extends typed_event_emitter_1.EventEmitter {
    constructor(options) {
        ...
        this._serverProperties = json_1.klona(ServerProperties_1.defaultServerProperties);
        ...
        this._logger = logger_1.createLogger({ name: 'ircv3', emoji: true, ...logger });
        ...
        if (channels) {
            this.addInternalListener(this.onRegister, async () => {
                const resolvedChannels = await shared_utils_1.resolveConfigValue(channels);
        ...
        this.onTypedMessage(Commands_1.CapabilityNegotiation, async ({ params: { subCommand, capabilities } }) => {
            ...
            switch (subCommand.toUpperCase()) {
                case 'NEW': {
                    ...
                    const capList = shared_utils_1.arrayToObject(caps, (part) => {
                        if (!part) {
                            return {};
                        }
                        const [cap, param] = shared_utils_1.splitWithLimit(part, '=', 2);
                        return {
                            [cap]: {
                                name: cap,
                                param: param || true
        ...
        this.onTypedMessage(Numerics_1.Reply005Isupport, ({ params: { supports } }) => {
            const newFeatures = shared_utils_1.arrayToObject(supports.split(' '), (part) => {
                const [support, param] = shared_utils_1.splitWithLimit(part, '=', 2);
        ...
        this.onTypedMessage(Commands_1.PrivateMessage, msg => {
            ...
            const ctcpMessage = StringTools_1.decodeCtcp(content);
    ...
    async getPassword(currentPassword) {
        return currentPassword;
    }
    registerCoreMessageTypes() {
        shared_utils_1.forEachObjectEntry(MessageTypes.Commands, (type) => {
            this.registerMessageType(type);
        });
        shared_utils_1.forEachObjectEntry(MessageTypes.Numerics, (type) => {
            this.registerMessageType(type);
        });
    }
    async _negotiateCapabilityBatch(capabilities) {
        return Promise.all(capabilities
            .filter(list => list.length)
            .map(async (capList) => this._negotiateCapabilities(capList)));
    }
    async _negotiateCapabilities(capList) {
        const mappedCapList = shared_utils_1.arrayToObject(capList, cap => ({
            [cap.name]: cap
        }));
            ...
            try {
                const [password] = await Promise.all([
                    ...
                    this.sendMessageAndCaptureReply(Commands_1.CapabilityNegotiation, {
                        ...
                        this._supportsCapabilities = true;
                        const capLists = capReply.map(line => shared_utils_1.arrayToObject(line.params.capabilities.split(' '), (part) => {
                            if (!part) {
                                return {};
                            }
                            const [cap, param] = shared_utils_1.splitWithLimit(part, '=', 2);
                ...
                this.quit().catch(() => this._logger.debug(`Error while disconnecting after password error: ${e.message}`));
            }
        });
